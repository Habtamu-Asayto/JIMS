...
public class ReportMeritizazandabetuta implements Serializable {
    String year;
 ...
 int[][] Meritizazandabetutacatagory;
 int [][]malefemale;
 
         String []catagory={"የህግ ምክር","የውል ረቂቅ ምርመራ","የውል ረቂቅ ዝግጂት"};   
         
         private List<Meritizazandabetuta> allMeritizazandabetuta;
         
    @EJB
    private com.database.MeritizazandabetutaFacade ejbFacade;

    public List<Meritizazandabetuta> getallMeritizazandabetuta() {
        return allMeritizazandabetuta;
    }

    public void setallMeritizazandabetuta(List<Meritizazandabetuta> allMeritizazandabetuta) {
        this.allMeritizazandabetuta = allMeritizazandabetuta;
    }
    ...
    public void init() {
        ...
        allMeritizazandabetuta=ejbFacade.findAll();
   ...
   // String dateInEthiopian = dtEthiopic.getDayOfMonth()<10 ?"0"+dtEthiopic.getDayOfMonth():dtEthiopic.getDayOfMonth() + "/" + (dtEthiopic.getMonthOfYear()<10)? "0"+dtEthiopic.getMonthOfYear():dtEthiopic.getMonthOfYear()  + "/" + dtEthiopic.getYear();
    ...
    private void createBarModel() throws ClassNotFoundException, SQLException {
      ...
      // barModelbasedonleve=initBarModel3();
        ...
        barModel.setTitle("መሪ ትዕዛዝ ፤አቤቱታ፤የህግ ምክር ፤ የውል ረቂቅ ምርመራ እና የውል ረቂቅ ዝግጂት አጠቃላይ የተከፈቱ ፋይሎች  =>"+year);
       ...
       System.out.println("Bar model Set up is done here ");
        ...
        yAxis.setMax(10);
        /////female male 
          barModemalefemale.setTitle("መሪ ትዕዛዝ ፤አቤቱታ፤የህግ ምክር ፤ የውል ረቂቅ ምርመራ እና የውል ረቂቅ ዝግጂት አጠቃላይ አገልግሎት የተሰጣቸው  ፋይሎች"+year);
    ...
    private BarChartModel initBarModel() throws ClassNotFoundException, SQLException {
       ...
       // BarChartModel modelfemalemale=new BarChartModel();
        ...
        Meritizazandabetutacatagory   =new int[datesgenerated.length][catagorytypes.length];
                     ...
                     //  System.out.println("Meritizazandabetuta data ====> 4"+daystotal.length);      
                             try
        {
			   for(Meritizazandabetuta dr :allMeritizazandabetuta )   
                               
     {
         // System.out.println("Meritizazandabetuta data ====> 3"+dr.getDateofarrival()); 
     String dataedata[]=dr.getMezgebuyetekefetebetken().split("/");
  ...
  if(year.equalsIgnoreCase( dataedata[2])){
               System.out.println("There is Year Generation===>"+dataedata[2]);
                             for(int i=0;i<datesgenerated.length;i++){
                              if(datesgenerated[i].equalsIgnoreCase(dr.getMezgebuyetekefetebetken())){
                                
                                  for(int j=0;j<catagorytypes.length;j++){
                                      
                                   if(dr.getYefayluaynet().equalsIgnoreCase(catagorytypes[j]))
                                                  {
                                       System.out.println("date ===>"+dataedata[2]+"how file is closed"+dr.getYefayluaynet());                 
                                       Meritizazandabetutacatagory[i][j]=Meritizazandabetutacatagory[i][j]+1;                                      
                                                   }
                                                }    
                                   
                          
                          daystotal[i]= daystotal[i] + 1; //caclulating total Meritizazandabetuta for each colleage
   ...
   projecttypesseries[j].set(datesgenerated[i], Meritizazandabetutacatagory[i][j] );
    ...
    private void createPieModel1() {
        ...
        pieModel1.setTitle("መሪ ትዕዛዝ ፤አቤቱታ፤የህግ ምክር ፤ የውል ረቂቅ ምርመራ እና የውል ረቂቅ ዝግጂት አጠቃላይ የተከፈቱ ፋይሎች =>"+year);
     ...
     private BarChartModel initBarModel2() throws ClassNotFoundException, SQLException {
       BarChartModel modelfemalemale=new BarChartModel();
        ...
        String yefayluaynet[]={ "የህግ ምክር","የውል ረቂቅ ምርመራ","የውል ረቂቅ ዝግጂት"};
      malefemale= new int[datesgenerated.length][yefayluaynet.length];
                        ...
                        for(Meritizazandabetuta dr :allMeritizazandabetuta )    
                        {
                            ...
                            String date[]=dr.getAgelgilotyetesetebetken().split("/");
             ...
             for(int i=0;i<datesgenerated.length;i++){ 
                   ...
                   //  System.out.println("The year is"+datesgenerated[i]+" and  school is ====>"+dr.getDateofarrival() +" ICU admission is "+dr.getSymptomspresent());
                       ...
                       { 
                          ...
                          if((datesgenerated[i].equalsIgnoreCase(dr.getAgelgilotyetesetebetken())) && (dr.getYefayluaynet().equalsIgnoreCase(yefayluaynet[t]) )){
     ...
     int totalforlevels[]={0};
      ...
      malefemale= new int[datesgenerated.length][yefayluaynet.length];
                        ...
                        for(Meritizazandabetuta dr :allMeritizazandabetuta )    
                        {
                            ...
                            String date[]=dr.getMezgebuyetekefetebetken().split("/");
...
for(int i=0; i<strArray.length;i++) {
    ...
    public ArrayList<MonthsandDataFeedcovid> getDatesgeneratedwithdata() {
      ...
      int  valuesbasedondifferentcase [][] = new int [datesgenerated.length][5];
        ...
        for(Meritizazandabetuta t : allMeritizazandabetuta)
        {
         /*
            for(int i=0;i<datesgenerated.length;i++)
            {
