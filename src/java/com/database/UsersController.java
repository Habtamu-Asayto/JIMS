...
public class UsersController implements Serializable {
    ...
    public void create(Users u, String caseid,String status) {
         System.out.println("The New User Will be created"+u.getFirtstname());
        this.selected=u;
        this.selected.setCaseid(caseid);
        this.selected.setStatus(status);
        persist(PersistAction.CREATE, ResourceBundle.getBundle("/Bundle").getString("UsersCreated"));
        if (!JsfUtil.isValidationFailed()) {
            items = null;    // Invalidate list of items to trigger re-query.
        }
    }
 public void create(String caseid,String status) {
     
      FacesContext context = FacesContext.getCurrentInstance();
        String viewId = context.getViewRoot().getViewId();
        PrimeFaces.current().multiViewState().clearAll(viewId, true);
       System.out.println("The Wizard is called ===> ");
        this.selected.setCaseid(caseid);
        this.selected.setStatus(status);
        persist(PersistAction.CREATE, ResourceBundle.getBundle("/Bundle").getString("UsersCreated"));
        if (!JsfUtil.isValidationFailed()) {
